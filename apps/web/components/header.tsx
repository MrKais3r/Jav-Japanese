...
export function Header() {
    ...
    return (
        <div className="flex flex-col gap-4 w-full max-w-6xl mx-auto mb-8">
            <div className="flex flex-col sm:flex-row items-center justify-between py-4 px-4 sm:px-6 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 shadow-xl transition-all hover:border-pink-500/30 gap-4 sm:gap-0">
                {/* Logo & Rank */}
                <div className="flex items-center gap-6">
                    <Link href="/" className="group flex flex-col items-center sm:items-baseline cursor-pointer">
                        <div className="text-3xl font-bold flex items-center align-middle gap-1 tracking-tight transition-transform group-hover:scale-[1.02]">
                            ...
                            <span className="text-transparent bg-clip-text bg-gradient-to-br from-pink-400 to-rose-600 text-4xl mt-1 text-center font-extrabold drop-shadow-[0_0_15px_rgba(244,114,182,0.5)]">
                                Jav
                            </span>
                        </div>
                    </Link>
                    ...
                    <div className="hidden md:flex flex-col gap-1">
                        <div className={`text-[10px] uppercase font-black tracking-widest ${currentRank?.color || "text-zinc-400"} flex items-center gap-1`}>
                            <Trophy size={10} /> {currentRank?.name || "Newbie"}
                        </div>
                        <div className="w-32 h-1.5 bg-white/10 rounded-full overflow-hidden">
                            <div 
                                className="h-full bg-linear-to-r from-pink-500 to-rose-500 transition-all duration-1000"

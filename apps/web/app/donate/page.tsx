...
export default function DonatePage() {
  ...
  return (
    <div className="min-h-dvh w-full flex flex-col items-center justify-start gap-10 p-4 md:p-8 relative overflow-hidden bg-black text-white pb-24">
        ...
        <Card className="border-0 bg-white/[0.03] backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden ring-1 ring-white/10 transition-all hover:ring-white/20">
          <CardHeader className="text-center border-b border-white/5 pb-8 pt-10">
            ...
            <CardTitle className="text-4xl md:text-5xl font-extrabold tracking-tight">
              Support <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-rose-400">Nihongo Jav</span>
            </CardTitle>
            ...
            <div className="w-full grid grid-cols- md:grid-cols-1 gap-6">
              {CRYPTO_OPTIONS.map((crypto, index) => {
                ...
                return (
                  ...
                  >
                    {/* Glow effect */}
                    <div className={`absolute -right-20 -top-20 w-40 h-40 bg-gradient-to-br ${crypto.color} opacity-10 blur-[50px] group-hover:opacity-20 transition-opacity pointer-events-none`} />

                    <div className="flex items-center justify-between relative z-10">
                      <div className="flex items-center gap-3">
                        <span className={`font-bold text-xl md:text-2xl ${crypto.textColor}`}>{crypto.name}</span>
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${crypto.bgColor} ${crypto.textColor} ${crypto.borderColor} border`}>
                          {crypto.network}
                        </span>
                      </div>

                      <button
                        onClick={() => setActiveQr(isShowingQr ? null : index)}
                    ...
                    <div className="mt-auto pt-2">
                      <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-2">Address / ID</div>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => copy(crypto.address, crypto.name)}
      ...
      <div
        className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 ${copyMessage ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-95 pointer-events-none"
          }`}
      >
